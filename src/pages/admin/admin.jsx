...
function Admin() {
  ...
  const isProdutoCadastro =
    pathname.startsWith("/admin/produtos/cadastrar") || pathname.startsWith("/admin/produtos/editar/");

  useEffect(() => {
    const rotaConhecida =
      pathname === "/admin" || isDashboard || isUsuarios || isCategorias || isPedidos || isProdutos;

    if (!rotaConhecida || pathname === "/admin") {
      navigate("/admin/produtos", { replace: true });
    }
  }, [pathname, isDashboard, isUsuarios, isCategorias, isPedidos, isProdutos, navigate]);
